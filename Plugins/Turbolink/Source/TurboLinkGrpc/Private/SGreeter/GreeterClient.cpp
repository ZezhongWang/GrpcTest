...
FGrpcContextHandle UStreamServiceClient::InitLotsOfReplies()
{
	FGrpcContextHandle handle = Service->TurboLinkManager->GetNextContextHandle();
	auto context = UGrpcClient::MakeContext<GrpcContext_StreamService_LotsOfReplies>(handle);
	context->RpcContext = UTurboLinkGrpcManager::Private::CreateRpcClientContext();
	return context->GetHandle();
}

void UStreamServiceClient::LotsOfReplies(FGrpcContextHandle Handle, const FGrpcGreeterHelloRequest& Request)
{
	auto context = UGrpcClient::GetContext(Handle);
	if (context != nullptr)
	{
		auto contextLotsOfReplies = StaticCastSharedPtr<GrpcContext_StreamService_LotsOfReplies>(*context);
		contextLotsOfReplies->Call(Request);
	}
}
FGrpcContextHandle UStreamServiceClient::InitBidiHello()
{
	FGrpcContextHandle handle = Service->TurboLinkManager->GetNextContextHandle();
	auto context = UGrpcClient::MakeContext<GrpcContext_StreamService_BidiHello>(handle);
	context->RpcContext = UTurboLinkGrpcManager::Private::CreateRpcClientContext();
	context->Init();
	return context->GetHandle();
}

void UStreamServiceClient::BidiHello(FGrpcContextHandle Handle, const FGrpcGreeterHelloRequest& Request)
{
	auto context = UGrpcClient::GetContext(Handle);
	if (context != nullptr)
	{
		auto contextBidiHello = StaticCastSharedPtr<GrpcContext_StreamService_BidiHello>(*context);
		contextBidiHello->Call(Request);
	}
}
void UStreamServiceClient::TryCancel(FGrpcContextHandle Handle)
{
	auto context = UGrpcClient::GetContext(Handle);
	if (context != nullptr)
	{
...
void UStreamServiceClient::Shutdown()
{
	OnLotsOfRepliesResponse.Clear();
	OnBidiHelloResponse.Clear();
	Super::Shutdown();
}
