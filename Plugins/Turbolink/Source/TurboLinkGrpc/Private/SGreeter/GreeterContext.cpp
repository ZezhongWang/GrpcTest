...
GrpcContext_StreamService_LotsOfReplies::GrpcContext_StreamService_LotsOfReplies(FGrpcContextHandle _Handle, UGrpcService* _Service, UGrpcClient* _Client)
	: Super(_Handle, _Service, _Client)
{
}

void GrpcContext_StreamService_LotsOfReplies::Call(const FGrpcGreeterHelloRequest& Request)
{
	check(GetState() == EGrpcContextState::Ready);
	UpdateState(EGrpcContextState::Initialing);
	...
	UStreamService* service = (UStreamService*)Service;
	RpcReaderWriter = service->d->Stub->AsyncLotsOfReplies(RpcContext.get(), rpcRequest, service->TurboLinkManager->d->CompletionQueue.get(), InitialTag);
}
void GrpcContext_StreamService_LotsOfReplies::OnRpcEvent(bool Ok, const void* EventTag)
{
	Super::OnRpcEventInternal(Ok, EventTag, [this](const FGrpcResult& _Result, ::Greeter::HelloResponse* _RpcResponse) {
		UStreamServiceClient* client = (UStreamServiceClient*)(this->Client);
		if (!(client->OnLotsOfRepliesResponse.IsBound())) return;

		FGrpcGreeterHelloResponse response;
		if (_RpcResponse) {
			GRPC_TO_TURBOLINK(_RpcResponse, &response);
		}
		client->OnLotsOfRepliesResponse.Broadcast(Handle, _Result, response);
	});
}

GrpcContext_StreamService_BidiHello::GrpcContext_StreamService_BidiHello(FGrpcContextHandle _Handle, UGrpcService* _Service, UGrpcClient* _Client)
	: Super(_Handle, _Service, _Client)
{
}

void GrpcContext_StreamService_BidiHello::Init()
{
	check(GetState() == EGrpcContextState::Ready);
	UpdateState(EGrpcContextState::Initialing);

	UStreamService* service = (UStreamService*)Service;
	RpcReaderWriter = service->d->Stub->AsyncBidiHello(RpcContext.get(), service->TurboLinkManager->d->CompletionQueue.get(), InitialTag);
}

void GrpcContext_StreamService_BidiHello::Call(const FGrpcGreeterHelloRequest& Request)
{
	::Greeter::HelloRequest rpcRequest;
	TURBOLINK_TO_GRPC(&Request, &rpcRequest);
...
void GrpcContext_StreamService_BidiHello::OnRpcEvent(bool Ok, const void* EventTag)
{
	Super::OnRpcEventInternal(Ok, EventTag, [this](const FGrpcResult& _Result, ::Greeter::HelloResponse* _RpcResponse) {
		UStreamServiceClient* client = (UStreamServiceClient*)(this->Client);
		if (!(client->OnBidiHelloResponse.IsBound())) return;

		FGrpcGreeterHelloResponse response;
		if (_RpcResponse) {
			GRPC_TO_TURBOLINK(_RpcResponse, &response);
		}
		client->OnBidiHelloResponse.Broadcast(Handle, _Result, response);
	});
}
